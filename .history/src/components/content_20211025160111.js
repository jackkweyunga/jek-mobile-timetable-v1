...
export const drawTimeTable = async (fac, ndays = 5, type = 'Seminar') => {
    // console.log(fac);
                        ...
                        for (let period of course.periods) {
                            if ((period.day.slice(0, 3) === days[i]) && (period.type == type)) {
                                ...
                                if (k + 8 === start || k + 8 === end) {
                                    ...
                                    checkForCollision(`${course.code}-${period.type}`, tRange(start, end), days[i]);
...
var int_container  = document.createElement('div').appendChild(top_content).appendChild(main_content)

content.innerHTML = int_container.innerHTML;
content.appendChild(int_container)
