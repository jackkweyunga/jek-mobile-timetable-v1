                        ...
                        for (let period of data.events) {
                            if ((period.day.slice(0, 3) === days[i]) && (period.type == type)) {
                                ...
                                if (k + 8 === start || k + 8 === end) {
                                    ...
                                    times[days[i]][`${period.cogroupde}-${period.type}`] = tRange(start, end);

                                    checkForCollision(`${period.code}-${period.type}`, tRange(start, end), days[i]);

                                    let bgcolor = "#470024";

                                    for (let color of bgcolors) {
                                        if (period.code === color.name) {
                                            bgcolor = color.color;
                                        }
                                    }
                                    ...
                                    lb.innerHTML = `
                                        ...
                                        ">
                                        ${period.code} ${period.type} <br>${start} - ${end}
                                        </div>
                                        `;
                                    lb.style = `
                                        position: absolute;
