...
(async function () {
    ...
    for (let el of lists) {
      ...
      <li class='list-group-item'>
        ...
        for (let t of el["tags"]) {
          li += `<button style="font-weight: 100;" class="m-1 text-white btn btn-sm bg-secondary"> ${t} </button>`
        }
  ...
  var filterProgrammes = function (event) {
    ...
    var filtered_programmes = programmes.filter(function (programmes) {
      programmes = programmes.toLowerCase();
      return programmes.indexOf(keyword) > -1;
    });
