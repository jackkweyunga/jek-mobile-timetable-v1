...
var SelectionListener = async (pname, pyear) => {
  startLoading();
  drawTimeTable({"name":pname, "year":pyear}, ndays, Ttype).then(() => {
    stopLoading();
  });
    ...
    for (let el of lists) {
      ...
      <li class='list-group-item'>
        ...
        for (let t of el["tags"]) {
          li += `<button onclick="SelectionListener(${el["name"]}, ${t})" style="font-weight: 100;" class="m-1 shadow-sm text-link btn btn-sm bg-white"> ${t} </button>`
        }
