...
window.onstorage = () => {
  console.log(JSON.parse(window.localStorage.getItem('selected-programme')));
}
    ...
    for (let el of lists) {
      ...
      <li class='list-group-item'>
        ...
        for (let t of el["tags"]) {
          ...
          var json_data = {"name":el["name"], "id":el["id"], "year":t}

          li += `<button onclick="window.localStorage.setItem('selected-programme', '${JSON.stringify(json_data)}');" style="font-weight: 100;" class="m-1 shadow-sm text-link btn btn-sm bg-white"> ${t} </button>`
        }
